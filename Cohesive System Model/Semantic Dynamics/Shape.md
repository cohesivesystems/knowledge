...
Shape is not the same as [[Value]]. A value is the concrete structured data. Shape is the declared structure under which that value is understood.

Shape is not the same as [[State]]. State is condition or configuration. Shape determines which dimensions of state are made visible, writable, comparable, or required for a particular [[Observation|observation]], [[Query|query]], [[Command|command]], [[Projections|projection]], or [[Transition|transition]].
...
Related concepts: [[Value]], [[Observation]], [[State]], [[Observable]], [[Query]], [[Command]], [[Transition]], [[Projections]], [[Boundaries]], [[Naturality]], [[Equivalence vs Equality]].
