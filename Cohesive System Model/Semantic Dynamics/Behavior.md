...
For an [[Entity]], behavior can be viewed as both an event schedule and a state history. The event schedule emphasizes occurrences and ordering. The state history emphasizes samples of state at versions, times, or cuts through the behavior.

Events do not accumulate into behavior by themselves. An [[Observer]] interprets an event schedule through an accumulator, integrator, scanner, or transition function, then supplies a hold or interpolation function that maps the resulting history to values over time:
...
Related concepts: [[Value]], [[State]], [[Observation]], [[Event]], [[Event-State Duality]], [[Algebras and Coalgebras]], [[Fixed Points and Recursion]], [[Trace and Feedback]], [[Entity]], [[Version]], [[Flows|Flow]].
