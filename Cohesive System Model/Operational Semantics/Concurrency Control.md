...
Concurrency control depends on the kind of state machine being modeled.
...
Event schedules and state histories are therefore related by [[Event-State Duality]], but not interchangeable. A linear entity history can be represented as alternating events and state versions. A non-sequential history may require partial orders, parent links, branch heads, merge events, or conflict records. The concurrency mechanism must match the history shape the system intends to preserve.

Related concepts: [[Command]], [[Transition]], [[Version]], [[Entity]], [[Event]], [[State]], [[Event-State Duality]], [[Event Sourcing]], [[CRDTs]], [[Behavior]], [[Realization]], [[Ordering]], [[Actor Systems]], [[Storage Systems]].
