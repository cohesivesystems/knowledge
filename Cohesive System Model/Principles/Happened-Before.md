...
If neither `A` happened before `B` nor `B` happened before `A`, the occurrences are [[Concurrency|concurrent]] relative to this relation. When an observer has only partial evidence about happened-before, the absence of an established direction is boundary-relative ordering knowledge rather than proof that no unobserved dependency exists.
