...
Capacity planning is an architecture and operational practice for relating expected demand, [[Service Levels|service-level objectives]], failure assumptions, and provisioning lead times to the effective capacity and reserve a system must have available.
...
Headroom is capacity intentionally left unused under an ordinary planning condition. Its amount should follow from variability, forecast error, actuation delay, workload growth, and the chosen [[Service Levels|service objectives]] rather than one universal utilization target.
